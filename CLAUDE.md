...
whichever conf is in use, and `cmdlinecheck` for reading `emos.system=` off
the cmdline the controller's packer stamped it onto — a misparse there mounts
a different FireOS userspace than the image was built beside, and boots. They all `#include init.c` whole and drive the real
...
image was built beside). CI runs all five, builds the init for aarch64
