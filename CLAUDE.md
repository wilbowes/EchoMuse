...
Configurable parameters: `vadThreshold`, `vadSpeechMs`, `vadSilenceMs`, `owwThreshold`, `owwModel`, `owwSpeexNs`, `adcDigitalGain`, `adcMicpga`, `micGainDb`, `startupVolume`, `beamAngle`, `beamformingEnabled`, `aecEnabled`, `aecDelayMs`, `aecTailMs`, `agcEnabled`, `nsAsr`, `bargeInEnabled`, `bargeInThreshold`, `bleProxyEnabled`, `eqBands`, `eqLoudness`, `limiterEnabled`, `limiterThreshold`, `limiterRelease`, `bassGuardEnabled`, `bassGuardDb`, `ledScene`, `ledListenColor`, `ledThinkColor`, `meterAttack`, `meterDecay`, `meterFloor`, `meterGamma`, `meterRef`, `meterCurve`, `wakeArbitrationMs`, `duckDb`, `buttonSingleTapEvent`, `buttonMultiTapMs`, `wakeSound`, `owwOnDevice` and `saveUtterances` (the last two are controller-consumed for scoping purposes, though `owwOnDevice` IS acted on by the device; `saveUtterances`, `wakeArbitrationMs` and the two `button*` keys are ignored by the device, because that work happens controller-side.

**The seven output-chain keys changed sides in 3.0.0.** `eqBands`, `eqLoudness`, `limiter*` and `bassGuard*` are now consumed **by the device**, post-mix at the ALSA write (`internal/outchain`), and the controller stands down for any device announcing `output_chain`. Before that they were controller-only, and this list said "the five output-chain keys" until 2026-08-22 — which implied `eqBands`/`eqLoudness` were already acted on device-side when nothing in `device/` referenced any of the seven. Both mirrors still matter: the controller keeps its own chain for firmware without the capability, so `em_api._apply_live_config` and `em_controller.handle_control` must both carry them (`tests/test_config_mirrors.py`). See `docs/audio-states.md` §8.

`wakeSound` is device-consumed too, with one exception: on the controller-detected wake path the controller sends `play_cue`, so it is mirrored controller-side as well).
