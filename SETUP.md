...
✅ Acoustic echo cancellation (v2.7.3, working since v2.7.7, default OFF) — speexdsp canceller on the whole mic path; reference tapped at the speaker ALSA write (same codec clock, no drift). Keep aecDelayMs at 0 (measured; higher values are non-causal — see v2.7.7). Converges to ~14dB per response on hardware; `[aec] att=` telemetry in the device log shows live attenuation. Enable from the dashboard Microphones advanced section
...
- **Acoustic echo cancellation** ✅ — shipped v2.7.3, but actually functional as of v2.7.7 (a silent buffer-size bypass meant it never ran — see the v2.7.7 changelog). speexdsp, ~14dB per response measured on hardware. Future: per-beam-channel filter states so repeat turns start converged instead of re-adapting from 0dB; possibly speex residual echo suppression on the barge watcher path.
...
- **§3.2 Wake-word barge-in** ✅ — complete as of v2.7.6 (`bargeInEnabled`, default off; requires device AEC). With it on, the mic streams through TTS playback and a dedicated per-device OWW watcher scores voice_queue at bargeInThreshold (used as-is since v2.7.7 — deliberately *below* owwThreshold, ~0.10: speech-over-TTS scores are depressed ~25dB by the speaker's loudness while post-AEC self-echo scores only ~0.004); detection sets cancel_event (aborts streaming + drain sleep), sends the new `speaker_flush` control message (device discards buffered periods; ≤~170ms ALSA in-flight still plays), and the turn loop re-enters a fresh turn with the wake-word preroll discard. VAD-based barge-in (interrupt by just talking) remains explicitly out of scope.
